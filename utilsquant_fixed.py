...
class Quantize(torch.nn.Module):
  ...
  def forward(self, img):
    #print(img.size)
    # quantize hace algo aquí que seguramente tenga que ver con el modo 'P'
    #img.convert('RGB')
    quant = img.quantize(self.colors)
...
def train_for_classification(net, train_loader, test_loader, optimizer, 
  ...
  for e in range(1,epochs+1):  
    ...
    for i, data in enumerate(train_loader):
      ...
      # Reducción entrópica_2
      indices_clasificacion_incorrecta = []
      computo = 9
      copy_x = torch.clone(X)
      copy_x.to(device)
      data_2 = DataLoader(X, batch_size=1)
      for i in [2**j for j in range(1,9)]:
        # En t_0 todas son mepis candidatas, después evoluciona.
        mepis_candidatas = torch.clone(copy_x)
        pre = transforms.Compose([transforms.ToPILImage(),
                                  Quantize(i),
                                  transforms.ToTensor()])
        l = []
        for z in data_2:
          ...
          _, C, H, W = z.shape
          z = z.view(C,H,W)
          # Añadir tensor quantizado a la lista
          l.append(pre(z).to(device))

        # Tensor para entregar al clasificador con las B imágenes quantizadas
        copy_x = torch.stack(l)
        #print(copy_x.shape)

        # Probar cuales son MEPIs pasando por el clasificador
        out_dict_2 = net(copy_x)
        ...
        # Tensor índices
        # List to cuda
        tensor_indices_clasificacion_incorrecta = torch.LongTensor(indices_clasificacion_incorrecta).to(device)
        # Crear un tensor con las MEPIs obtenidas
        mepis_confirmadas = torch.index_select(mepis_candidatas, 0, tensor_indices_clasificacion_incorrecta)
        mepis_confirmadas.to(device)
        copy_x.index_copy_(0, tensor_indices_clasificacion_incorrecta, mepis_confirmadas)
    ...
    # END reducción entrópica quantization
