...
def train_for_classification(net, train_loader, test_loader, optimizer, 
  ...
  for e in range(1,epochs+1):  
    ...
    for i, data in enumerate(train_loader):
      ...
      copy_x.to(device)
      # TODO: Des-HARDCODEAR eso (32)
      lista_mascaras = [range_slicing_mask([32,32], a,b,a,b) for a,b in zip(range(1,32//2), reversed(range(32)))]
      while (len(indices_clasificacion_incorrecta)<batch_size and computo > 0 and indice_del_zip<batch_size):
        mepis_candidatas = torch.clone(copy_x)
        mepis_candidatas.to(device)

        # Operación de reducción entrópica
        ##copy_x = downsampling(copy_x, computo) # Downsampling
        copy_x *= lista_mascaras[indice_del_zip] # Cropping
        # Operación de reducción entrópica finalizada
